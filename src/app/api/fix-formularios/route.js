...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (mappings.length > 0) {
            ...
            // 💡 MEJORA: Mapeo directo de fallback si los mappings de la DB están corruptos/vacíos, 
            // asumiendo los nombres de campos más comunes.
            console.warn('No hay mappings válidos de DB. Intentando mapeo directo (nombre, email).');
            const fieldsToTry = ['nombre', 'email', 'name']; 
            const bodyKeys = Object.keys(body);

            fieldsToTry.forEach(targetKey => {
                // Buscar campos como 'nombre', 'Name', 'EMAIL', etc.
                const matchingKey = bodyKeys.find(key => 
                ...
                if (matchingKey) {
                    ...
                    const value = rawValue !== null && rawValue !== undefined ? rawValue.toString().trim() : '';
                    if (value.length > 0) {
                        // Asignar el valor a 'nombre' si la clave es 'nombre' o 'name', o a 'email'
                        if (targetKey === 'nombre' || targetKey === 'name') {
                            contactData.nombre = value;
                        } else if (targetKey === 'email') {
                            contactData.email = value;
                        }
                        console.log(`Mapped via Fallback: ${matchingKey} -> ${targetKey === 'email' ? 'email' : 'nombre'} = ${value}`);
                    }
                }
            });
        }
        ...
        if (!contactData.email && (body.Email || body.email)) {
             const emailValue = body.Email || body.email;
             contactData.email = emailValue.toString().trim().length > 0 ? emailValue.toString().trim() : null;
        }
