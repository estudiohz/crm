...
const EditFacturaPage = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            formaCobro: factura.formaCobro,
            vencimiento: new Date(factura.vencimiento).toISOString().split('T')[0],
            contactoId: factura.contactoId,
            clienteSearch: factura.contacto ? factura.contacto.nombre + ' ' + (factura.contacto.apellidos || '') : '',
            estado: factura.estado,
            informacionFiscal: factura.informacionFiscal || '',
          ...
          setClienteSearch(factura.contacto ? factura.contacto.nombre + ' ' + (factura.contacto.apellidos || '') : '');
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Factura actualizada con éxito:', updatedFactura);

      setMessage('¡Factura actualizada con éxito! Redireccionando...');
      setTimeout(() => {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-between pt-4">
              ...
              <div className="flex space-x-3">
                <button type="button" onClick={() => router.push('/facturas')} className="btn px-4 py-2 border border-slate-300 text-slate-700 rounded-md hover:bg-slate-100 transition-colors duration-200 font-semibold text-sm">Cancelar</button>
                <button type="submit" disabled={isSubmitting} className="btn bg-blue-600 text-white hover:bg-blue-700 px-4 py-2 rounded-md font-semibold text-sm transition-colors duration-200 shadow-md flex items-center space-x-1 disabled:opacity-50">
                  {isSubmitting ? (<><Icon icon="heroicons:arrow-path" className="w-5 h-5 animate-spin" /><span>Actualizando...</span></>) : (<><Icon icon="heroicons:check-circle" className="w-5 h-5" /><span>Actualizar Factura</span></>)}
                </button>
              </div>
            </div>
          </form>
